...
namespace Thinktecture.EntityFrameworkCore.Storage.NestedRelationalTransactionManagerTests
{
   ...
   {
      public BeginTransactionAsync([NotNull] ITestOutputHelper testOutputHelper)
      ...
      public async Task Should_create_new_child_transaction_from_root_after_another_child_is_completed([NotNull] Action<IDbContextTransaction> action)
      {
         var rootTx = await SUT.BeginTransactionAsync();
         var childTx = await SUT.BeginTransactionAsync();
