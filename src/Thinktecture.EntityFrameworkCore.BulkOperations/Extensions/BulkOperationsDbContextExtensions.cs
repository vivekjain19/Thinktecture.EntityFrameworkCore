...
public static class BulkOperationsDbContextExtensions
{
   ...
   public static IQueryable<T> ToScalarCollectionParameter<T>(this DbContext ctx, IReadOnlyCollection<T> values, bool applyDistinct = true)
   {
   ...
   public static IQueryable<T> ToComplexCollectionParameter<T>(this DbContext ctx, IReadOnlyCollection<T> objects, bool applyDistinct = true)
      where T : class
   {
      return ctx.GetService<ICollectionParameterFactory>()
